...
export function SeriesPage() {
  ...
  useEffect(() => {
    if (!apiKey) return;
    Promise.allSettled([
      tmdbApi.getTrending('tv', 'week', apiKey).then(setTrending),
      tmdbApi.getPopular('tv', apiKey).then(setPopular),
      tmdbApi.getTopRated('tv', apiKey).then(setTopRated),
      tmdbApi.getOnTheAir(apiKey).then(setOnAir),
    ]);
  }, [apiKey]);
  ...
  const handleApplyFilters = async (filters: {
    ...
    try {
      ...
      if (isOnlyPrizeActive && awardMovieIds) {
        // Fetch details for all award-winning IDs (as 'tv' in Series page, though database is primarily movies)
        const detailsPromises = awardMovieIds.map(async (movieId) => {
          try {
            return await tmdbApi.getDetails(movieId, 'tv', apiKey);
          } catch (e) {
            console.error(`Failed to fetch details for series ${movieId}:`, e);
            return null;
          }
        });
      ...
      console.error('Error discovering TV series:', err);
    } finally {
  ...
  const handleAddWatchlist = useCallback(async (e: React.MouseEvent, item: any) => {
    ...
    try {
      const details = await tmdbApi.getDetails(item.id, 'tv', apiKey);
  ...
  return (
    <div className="py-8 pb-20">
      <div className="px-6 lg:px-10 mb-6">
        ...
        <p className="text-muted-light mb-6">Binge-worthy shows await</p>
      ...
      <AnimatePresence mode="wait">
        {loadingFilters ? (
          ...
          >
            ...
            <p className="text-sm text-muted-light">Filtering TV series...</p>
          </motion.div>
        ) : filtersActive ? (
          <motion.div
            key="results"
          ...
          >
            ...
            {filteredResults.length === 0 ? (
              <div className="text-center py-16 bg-white/5 rounded-2xl border border-white/5">
                ...
                <p className="text-sm text-muted-light">No TV series found matching these criteria.</p>
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <ContentRow title="🔥 Trending This Week" items={trending} mediaType="tv" />
            <ContentRow title="📡 Currently On Air" items={onAir} mediaType="tv" />
            <ContentRow title="⭐ Top Rated" items={topRated} mediaType="tv" />
            <ContentRow title="🎭 Popular Shows" items={popular} mediaType="tv" />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
